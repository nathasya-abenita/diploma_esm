...
def compare_precipitation (path_model, varname_model, path_obs, varname_obs, model_name):
    ...
    cf = xr.plot.contourf(bias, ax=ax, x='lon', y='lat', transform=ccrs.PlateCarree(),
                        cbar_kwargs={'label': 'MBE (K)', 'orientation': 'horizontal', 'pad': 0.05}, 
    ...
    # Save
    plt.savefig(f'output/cmip6/t2m_mbe_{model_name}.png')
    plt.close()
    ...
    # Save
    plt.savefig(f'output/cmip6/t2m_r_{model_name}.png')
